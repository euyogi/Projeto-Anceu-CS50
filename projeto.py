...
# Para guardar o nome do curso que foi pesquisado, evita um bug em que poderia mostrar o nome de outro curso nos resultados, sem ter pesquisado.
curso = str()
...
def UI() -> None:
    ...
    global menu_filtros, menu_cursos, rotulo_status, barra_progresso, rotulo_resultado, opcoes_anos, botao_detalhes
    global grupo_var, nota_1, nota_2, nota_3, nota_4, nota_5, nota_6
    ...
    balao(info_opcoes_anos, 'A formatação dos documentos mudaram\n'
    ...
    botao_detalhes = ctk.CTkButton(master = frame_esquerdo,
                                   ...
                                   state = 'disabled')
    botao_detalhes.grid(row = 4, column = 1, pady = (0, 30), padx = (40, 0))
    ...
    botao_sair.grid(row = 4, column = 2, pady = (0, 30), padx = (15, 50))
    ...
    rotulo_grupo_do_curso.grid(row = 1, column = 0, columnspan = 2, pady = 31, padx = 32)
    ...
    balao(info_rotulo_grupo_do_curso, 'Link para o PDF com os\n'
                            'pesos de cada curso no "i"\n'
    ...
    rotulo_insira_as_notas.grid(row = 4, column = 0, columnspan = 2, pady = 31, padx = 32)
    ...
    botao_converter.grid(row = 10, column = 0, columnspan = 2, pady = 31, padx = 16)
    
    rotulo_nota_final = ctk.CTkLabel(master = frame_direito, text_color = CINZA, text = 'Final', font = FONTE)
    rotulo_nota_final.grid(row = 11, column = 0, pady = (0, 32), padx = (32, 5), sticky = 'w')
    ...
    nota_6.grid(row = 11, column = 1, pady = (0, 32), padx = (5, 32), sticky = 'w')
...
def balao(widget, texto: str) -> None:
    balao_info_opcoes_anos = Balloon(widget.master)
    balao_info_opcoes_anos.bind(widget, texto)
    
    # Muda a fonte, cor do texto e de fundo, e deixa o texto centralizado.
    balao_info_opcoes_anos.component("label").configure(font = ('Cascadia Code', 12),
...
# Mostra as informações inicias no rotulo_resultado.
def ver_informacoes_iniciais() -> None:
    adicionar_na_maior_caixa(
'''Olá, este programa mostra as notas dos candidatos aprovados na UNB pelo
acesso ENEM.
...
Você pode escolher o ano que deseja checar e também é possível ver todas
as notas no botão "Ver Detalhes"
...
def iniciar_pesquisa() -> None:
    global cotas_notas, dados, inscricoes_aprovados, curso
...
def ver_info() -> None:
    texto = (
'''* Este programa não tem nenhuma afiliação com a Universidade de
* Brasília. Os dados utilizados estão disponíveis para o público no
* seguinte url: https://www.cebraspe.org.br/

* PDF com mais informações sobre o Acesso Enem UNB 2022, incluindo
...
# Altera os URLs de acordo com o ano escolhido.
def mudar_ano(ano: str) -> None:
    global URL_DAS_NOTAS, URL_DOS_APROVADOS
...
def ver_resumo_das_notas_na_maior_caixa() -> None:
    ...
    adicionar_na_maior_caixa(f'Curso: {curso} \n'
                             f'Vagas Totais: {vagas_totais} \t\tAno: {opcoes_anos.get().strip()} \t\t1ª Chamada \n'
                            )
    ...
    for s in arange(len(SIMBOLO_COTAS), dtype = int):
        if s == 2:
            adicionar_na_maior_caixa('\nEscola Pública: \n')
        adicionar_na_maior_caixa(
                                f'Vagas: {vagas_cotas[s]:02d}; {NOMES_DAS_COTAS[s]}: Máx = {max(cotas_notas[SIMBOLO_COTAS[s]]):.2f}; '
