...
namespace ImGui
{
	...
	namespace Internal
	{
		...
		float CalcComboItemHeight(int item_count);
	...
	namespace Internal
	{
		...
		bool ComboAutoSelectEX(const char* combo_label, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ComboAutoSelectSearchCallback<T2> autoselect_callback, ImGuiComboFlags flags)
		{
			...
			if (!(g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint)) {
				...
				SetNextWindowSizeConstraints(ImVec2(0.0f, 0.0f), ImVec2(popup_width, CalcComboItemHeight(popup_item_count)));
			}
			...
			if (BeginListBox(listbox_name, ImVec2(items_max_width, CalcComboItemHeight(popup_item_count)))) {
		...
		bool ComboFilterEX(const char* combo_label, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ComboFilterSearchCallback<T2> filter_callback, ImGuiComboFlags flags)
		{
			...
			if (!(g->NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint)) {
				...
				const float popup_height = CalcComboItemHeight(popup_item_count) + ImGui::GetFontSize() + 2 * style.FramePadding.y + style.ItemSpacing.y; // Increment popup_item_count to account for the InputText widget
			...
			auto item_getter2 = [&](int index) -> const char* {
				return item_getter(std::span(items), combo_data->FilterStatus ? combo_data->FilteredItems[index].Index : index);
			};
			...
			if (BeginListBox(listbox_name, ImVec2(items_max_width, CalcComboItemHeight(popup_item_count)))) {
				...
				if (clicked_outside || IsKeyPressed(ImGuiKey_Escape)) {
					...
					if (combo_data->SetNewValue(combo_data->CurrentSelection < 0 ? item_getter(std::span(items), -1) : item_getter2(combo_data->CurrentSelection))) {
						selection_changed = true;
						selected_item = combo_data->CurrentSelection;
					}
					CloseCurrentPopup();
