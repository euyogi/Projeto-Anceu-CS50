#define _CRT_SECURE_NO_WARNINGS // Disable warnings about strncpy
...
namespace ImGui
{
    ...
    namespace Internal
    {
        ...
        float CalcComboItemHeight(int item_count)
        {
            ...
            return item_count < 0 ? FLT_MAX : (g->FontSize + g->Style.ItemSpacing.y) * item_count + (g->Style.WindowPadding.y * 2);
        }
            ...
            if (matched) {
                ...
                for (int i = 0; i < nextMatch; ++i) {
                    ...
                    if (currIdx > 0) {
                        ...
                        if ((curr >= -1 && curr <= 255 && neighbor >= -1 && neighbor <= 255) && ::islower(neighbor) && ::isupper(curr)) {
                            outScore += camelBonus;
                        }
