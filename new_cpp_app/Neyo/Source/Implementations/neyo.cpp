...
void	processStream(std::string&, size_t&, const char*, size_t);
...
void processStream(std::string& txt_container, size_t& txt_container_ridx, const char* stream, size_t stream_size) {
	...
	char last_4_chars[4]{}; // Armazena os últimos 4 caracteres vistos.
...
// Checa se um token de dois caracteres apareceu (e.g. BT ou ET).
...
// desempenho (quase negligente, exceto em last_4_chars em processStream(), esse não deve ser uma string por
// razões de desempenho, apesar de uma array funcionar sem tanta perda de desempenho).
